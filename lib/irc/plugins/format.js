...
module.exports = function () {
	return function (irc) {
		irc.format = format;
		irc.format.unhighlight = irc['format.unhighlight'];
		irc.format.get = irc['format.get'];

		irc.define('irc', 'format.unhighlight', function (message) {
			return message.split('').join(irc.format.zwsp);
		});

		irc.define('irc', 'format.get', function (name) {
			return format[name];
		});
