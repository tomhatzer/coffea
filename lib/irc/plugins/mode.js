...
module.exports = function () {
    return function (irc) {
        irc.define('irc', 'mode', function ircMode(target, flags, network, fn) {
        ...
        irc.define('irc', 'umode', function ircUMode(flags, network, fn) {
            irc.mode(irc.me, flags, network, fn);
        });

        irc.define('irc', 'chanmode', function ircChanmode(channel, mode, target, network, fn) {
            var parse = utils.parseTarget(target);
            if (!network) { network = parse.network; }
            target = parse.target;
        ...
        irc.define('irc', 'voice', function ircVoice(target, nicks, network, fn) {
            return irc.chanmode(target, "+v", nicks, network, fn);
        });

        irc.define('irc', 'devoice', function ircDevoice(target, nicks, network, fn) {
            return irc.chanmode(target, "-v", nicks, network, fn);
        });

        irc.define('irc', 'op', function ircOp(target, nicks, network, fn) {
            return irc.chanmode(target, "+o", nicks, network, fn);
        });

        irc.define('irc', 'deop', function ircDeop(target, nicks, network, fn) {
            return irc.chanmode(target, "-o", nicks, network, fn);
        });

        irc.define('irc', 'hop', function ircHop(target, nicks, network, fn) {
            return irc.chanmode(target, "+h", nicks, network, fn);
        });

        irc.define('irc', 'dehop', function ircDehop(target, nicks, network, fn) {
            return irc.chanmode(target, "-h", nicks, network, fn);
        });
