...
module.exports = function () {
    return function (irc) {
        irc.define('irc', 'nick', function ircNick(nick, network, fn) {
            if (!network) network = '0';
            this.networked_me[network] = this.getUser(nick, network); // TODO: core should allow protocols to store data in a stream
            this.me = this.networked_me[network];
            this.write('NICK ' + nick, network, fn);
        });

        irc.define('irc', 'getMe', function ircGetMe(network) {
            if (!network) network = '0';
